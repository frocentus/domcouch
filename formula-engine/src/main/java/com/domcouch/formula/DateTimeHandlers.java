...
final class DateTimeHandlers {
    ...
    static void register(java.util.Map<String, FunctionHandler> functions) {
        functions.put("BUSINESSDAYS", (ev, args, ctx) -> {
            ...
            if (args.size() > 3) for (Object o : Evaluator.toList(ev.eval(args.get(3), ctx))) { java.time.LocalDate ld = Evaluator.parseDate(Evaluator.toString(o)); if (ld != null) excludeDates.add(ld); }
            ...
            for (int i = 0; i < size; i++) {
                java.time.LocalDate s = Evaluator.parseDate(Evaluator.toString(startList.get(Math.min(i, startList.size() - 1))));
                java.time.LocalDate e = Evaluator.parseDate(Evaluator.toString(endList.get(Math.min(i, endList.size() - 1))));
        ...
        functions.put("DATE", (ev, args, ctx) -> {
            ...
            for (Object src : sources) { java.time.ZonedDateTime zdt = Evaluator.parseDateToZoned(Evaluator.toString(src)); result.add(zdt != null ? Evaluator.DT_FMT.format(zdt.toLocalDate().atStartOfDay(zdt.getZone())) : ""); }
            return result.size() == 1 ? result.getFirst() : result;
        });
        ...
        functions.put("ADJUST", (ev, args, ctx) -> {
            ...
            boolean inLocalTime = args.size() > 7 && "INLOCALTIME".equalsIgnoreCase(Evaluator.toString(ev.eval(args.get(7), ctx)));
            List<Object> sources = Evaluator.toList(dateVal); List<Object> result = new ArrayList<>();
            for (Object src : sources) {
                java.time.ZonedDateTime dt = Evaluator.parseDateToZoned(Evaluator.toString(src));
        ...
        functions.put("TIME", (ev, args, ctx) -> {
            ...
            List<Object> sources = Evaluator.toList(ev.eval(args.get(0), ctx)); List<Object> result = new ArrayList<>();
            for (Object src : sources) { java.time.ZonedDateTime zdt = Evaluator.parseDateToZoned(Evaluator.toString(src)); if (zdt == null) result.add(""); else result.add(Evaluator.DT_FMT.format(zdt.toLocalTime().atDate(java.time.LocalDate.of(1970, 1, 1)).atZone(zdt.getZone()))); }
        ...
        functions.put("TIMEMERGE", (ev, args, ctx) -> {
            ...
            for (int i = 0; i < size; i++) {
                java.time.ZonedDateTime d = Evaluator.parseDateToZoned(Evaluator.toString(dates.get(Math.min(i, dates.size() - 1))));
                java.time.ZonedDateTime t = Evaluator.parseDateToZoned(Evaluator.toString(times.get(Math.min(i, times.size() - 1))));
        ...
        functions.put("ZONE", (ev, args, ctx) -> { String td = args.isEmpty() ? null : Evaluator.toString(ev.eval(args.get(0), ctx)); try { return ctx.getTimeZoneOffset(td); } catch (ContextNotSupportedException e) { return java.time.ZoneId.systemDefault().getId(); } });
        ...
        functions.put("TIMETOTEXTINZONE", (ev, args, ctx) -> { String td = Evaluator.toString(ev.eval(args.get(0), ctx)), tz = Evaluator.toString(ev.eval(args.get(1), ctx)), fmt = args.size() > 2 ? Evaluator.toString(ev.eval(args.get(2), ctx)) : ""; try { return ctx.timeToTextInZone(td, tz, fmt); } catch (ContextNotSupportedException e) { return ""; } });
        functions.put("TIMEZONETOTEXT", (ev, args, ctx) -> { String tz = Evaluator.toString(ev.eval(args.get(0), ctx)), fmt = args.size() > 1 ? Evaluator.toString(ev.eval(args.get(1), ctx)) : ""; try { return ctx.timeZoneToText(tz, fmt); } catch (ContextNotSupportedException e) { return ""; } });
    }
}
