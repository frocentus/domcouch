...
public final class MiscHandlers {
    ...
    public static void register(java.util.Map<String, FunctionHandler> functions) {
        ...
        functions.put("ISAVAILABLE", (ev, args, ctx) -> { String n; if (args.getFirst() instanceof Expr.Variable v) n = v.name(); else n = Evaluator.convertToString(ev.eval(args.getFirst(), ctx)); return Evaluator.boolToNum(ctx.resolve(n) != null); });
        functions.put("ISUNAVAILABLE", (ev, args, ctx) -> { String n; if (args.getFirst() instanceof Expr.Variable v) n = v.name(); else n = Evaluator.convertToString(ev.eval(args.getFirst(), ctx)); return Evaluator.boolToNum(ctx.resolve(n) == null); });
        functions.put("ISNEWDOC", (ev, args, ctx) -> { try { return Evaluator.boolToNum(ctx.getDocumentUNID().isEmpty()); } catch (
                ContextNotSupportedException e) { return 1.0; } });
        functions.put("ISRESPONSEDOC", (ev, args, ctx) -> { Object p = ctx.resolve("PARENTUNID"); return Evaluator.boolToNum(p != null && !Evaluator.convertToString(p).isEmpty()); });
        ...
        functions.put("INHERITEDDOCUMENTUNIQUEID", (ev, args, ctx) -> ctx.resolve("PARENTUNID")!=null? Evaluator.convertToString(ctx.resolve("PARENTUNID")):"");
        ...
        functions.put("UNIQUE", (ev, args, ctx) -> { List<Object> s = Evaluator.toList(ev.eval(args.getFirst(),ctx)); java.util.LinkedHashSet<Object> seen = new java.util.LinkedHashSet<>(); for (Object o : s) seen.add(Evaluator.convertToString(o)); List<Object> r = new ArrayList<>(seen); return r.isEmpty()?"":r.size()==1?r.getFirst():r; });
        functions.put("MEMBER", (ev, args, ctx) -> { Object n = ev.eval(args.get(0),ctx); List<Object> h = Evaluator.toList(ev.eval(args.get(1),ctx)); for (int i=0;i<h.size();i++) if (Evaluator.convertToString(n).equals(Evaluator.convertToString(h.get(i)))) return (double)(i+1); return 0.0; });
        functions.put("IMPLODE", (ev, args, ctx) -> { List<Object> s = Evaluator.toList(ev.eval(args.get(0),ctx)); String sep = args.size()>1? Evaluator.convertToString(ev.eval(args.get(1),ctx)):" "; return s.isEmpty()?"":String.join(sep, s.stream().map(Evaluator::convertToString).toList()); });
        functions.put("SORT", (ev, args, ctx) -> { List<Object> s = new ArrayList<>(Evaluator.toList(ev.eval(args.get(0),ctx))); s.sort((a,b)-> Evaluator.convertToString(a).compareTo(Evaluator.convertToString(b))); return s.isEmpty()?"":s.size()==1?s.get(0):s; });
        ...
        functions.put("ISNULL", (ev, args, ctx) -> { Object v = ev.eval(args.get(0),ctx); return Evaluator.boolToNum(v==null||"".equals(Evaluator.convertToString(v))); });
        ...
        functions.put("FAILURE", (ev, args, ctx) -> args.isEmpty()?"": Evaluator.convertToString(ev.eval(args.getFirst(),ctx)));
        ...
        functions.put("LIKE", (ev, args, ctx) -> { Object str = ev.eval(args.get(0),ctx), pat = ev.eval(args.get(1),ctx); String ec = args.size()>2? Evaluator.convertToString(ev.eval(args.get(2),ctx)):null; return Evaluator.boolToNum(Evaluator.anyPairMatch(str, pat, (s, pattern)->{ StringBuilder re = new StringBuilder("^(?i)"); for (int i = 0; i<pattern.length(); i++) { char c = pattern.charAt(i); if (ec!=null&&!ec.isEmpty()&&pattern.startsWith(ec,i)) { i+=ec.length(); if (i<pattern.length()) re.append(java.util.regex.Pattern.quote(String.valueOf(pattern.charAt(i)))); continue; } if (c=='_') re.append('.'); else if (c=='%') re.append(".*"); else re.append(java.util.regex.Pattern.quote(String.valueOf(c))); } return s.matches(re.append("$").toString()); })); });
        ...
        functions.put("ISTIME", (ev, args, ctx) -> { List<Object> s = Evaluator.toList(ev.eval(args.get(0),ctx)); for (Object o : s) { if (o instanceof Number) return 0.0; if (Evaluator.parseDateToZoned(Evaluator.convertToString(o))==null) return 0.0; } return 1.0; });
        functions.put("TEXTTOTIME", (ev, args, ctx) -> { List<Object> s = Evaluator.toList(ev.eval(args.get(0),ctx)); List<Object> r = new ArrayList<>(); for (Object o : s) { java.time.ZonedDateTime z = Evaluator.parseDateToZoned(Evaluator.convertToString(o)); r.add(z==null?"":Evaluator.DT_FMT.format(z)); } return r.size()==1?r.get(0):r; });
        ...
        functions.put("TOTIME", (ev, args, ctx) -> { List<Object> s = Evaluator.toList(ev.eval(args.get(0),ctx)); List<Object> r = new ArrayList<>(); for (Object o : s) { java.time.ZonedDateTime z = Evaluator.parseDateToZoned(Evaluator.convertToString(o)); r.add(z==null?"":Evaluator.DT_FMT.format(z)); } return r.size()==1?r.get(0):r; });
        ...
        functions.put("UNAVAILABLE", (ev, args, ctx) -> Evaluator.boolToNum(ctx.resolve(Evaluator.convertToString(ev.eval(args.get(0),ctx)))==null));
        functions.put("ENVIRONMENT", (ev, args, ctx) -> { try { return ctx.getEnvironmentValue(Evaluator.convertToString(ev.eval(args.getFirst(),ctx))); } catch (ContextNotSupportedException e) { return ""; } });
        ...
        functions.put("ADDTOFOLDER", (ev, args, ctx) -> { try { ctx.addToFolder(Evaluator.convertToString(ev.eval(args.getFirst(),ctx))); return 1.0; } catch (ContextNotSupportedException e) { return 1.0; } });
        ...
        functions.put("GETFIELD", (ev, args, ctx) -> { Object v = ctx.resolve(Evaluator.convertToString(ev.eval(args.get(0),ctx))); return v!=null?v:""; });
        functions.put("CHECKFORMULASYNTAX", (ev, args, ctx) -> { String f = Evaluator.convertToString(ev.eval(args.get(0),ctx)); try { new Parser(Lexer.tokenize(f)).parse(); return "1"; } catch (FormulaParseException e) { return List.of(e.getMessage(),"1",String.valueOf(e.position+1),"1",String.valueOf(e.position+1),"1",f); } });
        functions.put("EVAL", (ev, args, ctx) -> ev.evalExpr(Evaluator.convertToString(ev.eval(args.get(0),ctx)),ctx));
