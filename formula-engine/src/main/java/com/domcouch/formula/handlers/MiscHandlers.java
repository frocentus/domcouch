...
public final class MiscHandlers {
    ...
    public static void register(java.util.Map<String, FunctionHandler> functions) {
        functions.put("TEXT", (ev, args, ctx) -> {
            ...
            String fmt = args.size() > 1 ? Evaluator.convertToString(ev.eval(args.get(1), ctx)) : null;
            List<Object> s = Evaluator.toList(val);
        ...
        functions.put("TEXTTONUMBER", (ev, args, ctx) -> {
            ...
            List<Object> r = new ArrayList<>();
            for (Object o : s) {
                String str = Evaluator.convertToString(o).trim();
                if (str.isEmpty()) {
                    r.add(0.0);
                    continue;
                }
                ...
                try {
                    ...
                    if (end > 0 && (str.charAt(0) == '-' || str.charAt(0) == '+' || Character.isDigit(str.charAt(0)))) {
                        try {
                            d = Double.parseDouble(str.substring(0, end));
        ...
        functions.put("ISNUMBER", (ev, args, ctx) -> {
            ...
            if (v instanceof List<?> l) {
                for (Object e : l) if (!(e instanceof Number)) return 0.0;
                return l.isEmpty() ? 0.0 : 1.0;
            }
            return 0.0;
        });
        functions.put("ISTEXT", (ev, args, ctx) -> {
            ...
            if (v instanceof List<?> l) {
                for (Object e : l) if (!(e instanceof String)) return 0.0;
                return l.isEmpty() ? 0.0 : 1.0;
            }
            return 0.0;
        });
        ...
        functions.put("ISAVAILABLE", (ev, args, ctx) -> {
            String n;
            if (args.getFirst() instanceof Expr.Variable(String name)) n = name;
            else n = Evaluator.convertToString(ev.eval(args.getFirst(), ctx));
            return Evaluator.boolToNum(ctx.resolve(n) != null);
        });
        ...
        functions.put("ISNEWDOC", (ev, args, ctx) -> {
            try {
                return Evaluator.boolToNum(ctx.getDocumentUNID().isEmpty());
            } catch (
                    ContextNotSupportedException e) {
                return 1.0;
            }
        });
        functions.put("ISRESPONSEDOC", (ev, args, ctx) -> {
            Object p = ctx.resolve("PARENTUNID");
            return Evaluator.boolToNum(p != null && !Evaluator.convertToString(p).isEmpty());
        });
        functions.put("NOTEID", (ev, args, ctx) -> {
            try {
                return "NT" + ctx.getDocumentUNID().substring(0, Math.min(8, ctx.getDocumentUNID().length()));
        ...
        functions.put("ATTACHMENTS", (ev, args, ctx) -> {
            try {
                return (double) ctx.getAttachmentCount();
            } catch (ContextNotSupportedException e) {
                return 0.0;
            }
        });
        functions.put("ELEMENTS", (ev, args, ctx) -> {
            ...
            if (v == null || (v instanceof String s && s.isEmpty())) return 0.0;
            return 1.0;
        });
        functions.put("COUNT", (ev, args, ctx) -> {
            Object v = ev.eval(args.getFirst(), ctx);
            if (v instanceof List l) return l.isEmpty() ? 1.0 : (double) l.size();
            return 1.0;
        });
        functions.put("ISMEMBER", (ev, args, ctx) -> {
            Object val = ev.eval(args.get(0), ctx), list = ev.eval(args.get(1), ctx);
            List<Object> l2 = Evaluator.toList(list);
            if (val instanceof List<?> l1) {
                for (Object e : l1) if (!l2.contains(e)) return 0.0;
                return l1.isEmpty() ? 0.0 : 1.0;
            }
            return Evaluator.boolToNum(l2.contains(val));
        });
        ...
        functions.put("REPLACE", (ev, args, ctx) -> {
            List<Object> s = Evaluator.toList(ev.eval(args.get(0), ctx)), f = Evaluator.toList(ev.eval(args.get(1), ctx)), t = Evaluator.toList(ev.eval(args.get(2), ctx));
            List<Object> r = new ArrayList<>();
            for (Object src : s) {
                int idx = f.indexOf(src);
                r.add(idx >= 0 && idx < t.size() ? t.get(idx) : src);
            }
            return r.size() == 1 ? r.getFirst() : r;
        });
        functions.put("MAX", (ev, args, ctx) -> {
            ...
            for (Expr arg : args)
                for (Object o : Evaluator.toList(ev.eval(arg, ctx))) max = Math.max(max, Evaluator.toNumber(o));
            return max == (int) max ? (double) (int) max : max;
        });
        functions.put("MIN", (ev, args, ctx) -> {
            double min = Double.POSITIVE_INFINITY;
            for (Expr arg : args)
                for (Object o : Evaluator.toList(ev.eval(arg, ctx))) min = Math.min(min, Evaluator.toNumber(o));
        ...
        functions.put("SUBSET", (ev, args, ctx) -> {
            List<Object> s = Evaluator.toList(ev.eval(args.get(0), ctx));
            int n = (int) Evaluator.toNumber(ev.eval(args.get(1), ctx));
            List<Object> r = new ArrayList<>();
            if (n > 0) for (int i = 0; i < n && i < s.size(); i++) r.add(s.get(i));
        ...
        functions.put("UNIQUE", (ev, args, ctx) -> {
            ...
            for (Object o : s) seen.add(Evaluator.convertToString(o));
            List<Object> r = new ArrayList<>(seen);
            return r.isEmpty() ? "" : r.size() == 1 ? r.getFirst() : r;
        });
        functions.put("MEMBER", (ev, args, ctx) -> {
            Object n = ev.eval(args.get(0), ctx);
            List<Object> h = Evaluator.toList(ev.eval(args.get(1), ctx));
            for (int i = 0; i < h.size(); i++)
                if (Evaluator.convertToString(n).equals(Evaluator.convertToString(h.get(i)))) return (double) (i + 1);
            return 0.0;
        });
        functions.put("IMPLODE", (ev, args, ctx) -> {
            List<Object> s = Evaluator.toList(ev.eval(args.get(0), ctx));
            String sep = args.size() > 1 ? Evaluator.convertToString(ev.eval(args.get(1), ctx)) : " ";
            return s.isEmpty() ? "" : String.join(sep, s.stream().map(Evaluator::convertToString).toList());
        });
        functions.put("SORT", (ev, args, ctx) -> {
            List<Object> s = new ArrayList<>(Evaluator.toList(ev.eval(args.getFirst(), ctx)));
            s.sort(Comparator.comparing(Evaluator::convertToString));
            return s.isEmpty() ? "" : s.size() == 1 ? s.getFirst() : s;
        });
        functions.put("IF", (ev, args, ctx) -> {
            int n = args.size();
            if (n == 0) return "";
            if (n == 1) {
                ev.eval(args.getFirst(), ctx);
                return "";
            }
            for (int i = 0; i + 1 < n; i += 2)
                if (Evaluator.isTruthy(ev.eval(args.get(i), ctx))) return ev.eval(args.get(i + 1), ctx);
        ...
        functions.put("RETURN", (ev, args, ctx) -> {
            throw new Evaluator.ReturnValue(ev.eval(args.getFirst(), ctx));
        });
        functions.put("ISNULL", (ev, args, ctx) -> {
            Object v = ev.eval(args.getFirst(), ctx);
            return Evaluator.boolToNum(v == null || "".equals(Evaluator.convertToString(v)));
        });
        functions.put("ISVALID", (ev, args, ctx) -> {
            try {
                return Evaluator.boolToNum(ctx.isDocumentValid());
            } catch (ContextNotSupportedException e) {
                return 1.0;
            }
        });
        ...
        functions.put("LIKE", (ev, args, ctx) -> {
            Object str = ev.eval(args.get(0), ctx), pat = ev.eval(args.get(1), ctx);
            String ec = args.size() > 2 ? Evaluator.convertToString(ev.eval(args.get(2), ctx)) : null;
            return Evaluator.boolToNum(Evaluator.anyPairMatch(str, pat, (s, pattern) -> {
                StringBuilder re = new StringBuilder("^(?i)");
                for (int i = 0; i < pattern.length(); i++) {
                    char c = pattern.charAt(i);
                    if (ec != null && !ec.isEmpty() && pattern.startsWith(ec, i)) {
                        i += ec.length();
                        if (i < pattern.length())
                ...
                return s.matches(re.append("$").toString());
            }));
        });
        ...
        functions.put("TONUMBER", (ev, args, ctx) -> Evaluator.map1(ev, args, ctx, s -> {
            try {
                return Evaluator.toNumber(s);
            } catch (Exception e) {
                return 0.0;
            }
        }));
        ...
        functions.put("ENVIRONMENT", (ev, args, ctx) -> {
            try {
                return ctx.getEnvironmentValue(Evaluator.convertToString(ev.eval(args.getFirst(), ctx)));
            } catch (ContextNotSupportedException e) {
                return "";
            }
        });
        ...
        functions.put("DBNAME", (ev, args, ctx) -> {
            try {
                return List.of(ctx.getServerName(), ctx.getDatabaseName());
            } catch (ContextNotSupportedException e) {
                return List.of("", "");
            }
        });
        ...
        functions.put("DOCLENGTH", (ev, args, ctx) -> {
            try {
                return (double) ctx.getDocumentSize();
            } catch (ContextNotSupportedException e) {
                return 0.0;
            }
        });
        ...
        functions.put("DOCLOCK", (ev, args, ctx) -> {
            ...
            String kw = Evaluator.convertToString(ev.eval(args.getFirst(), ctx));
            try {
                return switch (kw) {
                    case "LOCK" -> Evaluator.boolToNum(ctx.lockDocument());
                    case "UNLOCK" -> Evaluator.boolToNum(ctx.unlockDocument());
                    case "STATUS" -> ctx.getDocumentLockStatus();
                    case "LOCKINGENABLED" -> Evaluator.boolToNum(ctx.isDocumentLockingEnabled());
                    default -> "";
                };
            } catch (ContextNotSupportedException e) {
                return switch (kw) {
        ...
        functions.put("ADDTOFOLDER", (ev, args, ctx) -> {
            try {
                ctx.addToFolder(Evaluator.convertToString(ev.eval(args.getFirst(), ctx)));
                return 1.0;
            } catch (ContextNotSupportedException e) {
                return 1.0;
            }
        ...
        functions.put("CHECKFORMULASYNTAX", (ev, args, ctx) -> {
            ...
            try {
                new Parser(Lexer.tokenize(f)).parse();
                return "1";
        ...
        functions.put("DOWHILE", (ev, args, ctx) -> {
            ...
            } while (Evaluator.isTruthy(ev.eval(args.getLast(), ctx)));
            return 1.0;
        });
        functions.put("FOR", (ev, args, ctx) -> {
            if (args.size() < 4) return "";
            ev.eval(args.get(0), ctx);
            Object last = 1.0;
            while (Evaluator.isTruthy(ev.eval(args.get(1), ctx))) {
                for (int i = 3; i < args.size(); i++) last = ev.eval(args.get(i), ctx);
                ev.eval(args.get(2), ctx);
            }
            return last;
        });
        functions.put("TRANSFORM", (ev, args, ctx) -> {
            List<Object> list = Evaluator.toList(ev.eval(args.get(0), ctx));
            String vn = Evaluator.convertToString(ev.eval(args.get(1), ctx)).toUpperCase();
            Expr f = args.get(2);
            List<Object> r = new ArrayList<>();
            for (Object elem : list) {
                FormulaContext ec = new FormulaContext() {
                    @Override
                    public Object resolve(String n) {
                        return n.equals(vn) ? elem : ctx.resolve(n);
                    }

                    @Override
                    public void setField(String n, Object v) {
                        ctx.setField(n, v);
                    }

                    @Override
                    public void deleteField(String n) {
                        ctx.deleteField(n);
                    }
                    ...
                    public boolean lockDocument() {
                        return ctx.lockDocument();
                    }

                    @Override
                    public boolean unlockDocument() {
                        return ctx.unlockDocument();
                    }

                    @Override
                    public String getDocumentLockStatus() {
                        return ctx.getDocumentLockStatus();
                    }

                    @Override
                    public boolean isDocumentLockingEnabled() {
                        return ctx.isDocumentLockingEnabled();
                    }
                    ...
                    @Override
                    public String getEnvironmentValue(String n) {
                        return ctx.getEnvironmentValue(n);
                    }
                    ...
                    @Override
                    public void addToFolder(String n) {
                        ctx.addToFolder(n);
                    }

                    @Override
                    public java.util.List<Number> getTimeZoneOffset(String d) {
                        return ctx.getTimeZoneOffset(d);
                    }

                    @Override
                    public String getCanonicalTimeZone() {
                        return ctx.getCanonicalTimeZone();
                    }

                    @Override
                    public String timeToTextInZone(String d, String z, String fmt) {
                        return ctx.timeToTextInZone(d, z, fmt);
                    }

                    @Override
                    public String timeZoneToText(String z, String fmt) {
                        return ctx.timeZoneToText(z, fmt);
                    }
                };
                r.add(ev.eval(f, ec));
            }
        ...
        functions.put("SET", (ev, args, ctx) -> {
            String vn = Evaluator.convertToString(ev.eval(args.get(0), ctx)).toUpperCase();
            Object val = ev.eval(args.get(1), ctx);
            ev.tempScope.get().put(vn, val);
            return val;
        });
        functions.put("SETFIELD", (ev, args, ctx) -> {
            String fn = Evaluator.convertToString(ev.eval(args.get(0), ctx)).toUpperCase();
            Object val = ev.eval(args.get(1), ctx);
            try {
                ctx.setField(fn, val);
            } catch (ContextNotSupportedException e) {
